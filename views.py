...
def Profil(request, pk):
	...
	posts = Post.objects.filter(user=post.user)
	
	context = {'posts':posts, 'post': post,'base_template_name':base_template_name}
...
def DetailView(request, pk):
	if request.user.is_authenticated():
		...
		base_template_name = 'blog/visitor.html'
	post = get_object_or_404(Post, pk=pk)
	return render(request,'blog/detail.html',{'post':post,'base_template_name':base_template_name})
